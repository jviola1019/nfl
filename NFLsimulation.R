...
# Ensure the downstream HTML handoff object exists even if upstream comparison
# steps bail early. This prevents missing-object errors in the closing export
# guard logic when the simulation is sourced piecemeal.
if (!exists("moneyline_report_inputs", inherits = FALSE)) {
...
# Preserve comparison output for the HTML report once all data prep is complete
moneyline_report_inputs <- NULL
if (exists("cmp_blend") && !is.null(cmp_blend)) {
  ...
  message("Moneyline report inputs unavailable; blended market comparison not generated.")
}
...
moneyline_inputs <- get0("moneyline_report_inputs", ifnotfound = NULL, inherits = TRUE)
has_moneyline_inputs <- is.list(moneyline_inputs) && length(moneyline_inputs)
has_moneyline_report <- exists("moneyline_report", inherits = TRUE)

if (has_moneyline_inputs && has_moneyline_report) {
  ...
  if (!is.null(report_schedule)) {
    required_elements <- c("comparison", "join_keys")
    missing_elements <- setdiff(required_elements, names(moneyline_inputs))

    if (length(missing_elements)) {
      warning(
        sprintf(
          "moneyline_report_inputs is missing required elements: %s; HTML export skipped.",
      ...
      report_path <- tryCatch(
        moneyline_report(
          market_comparison_result = moneyline_inputs$comparison,
          schedule = report_schedule,
          join_keys = moneyline_inputs$join_keys,
          vig = 0.10,
          title = report_title,
          verbose = TRUE,
          auto_open = TRUE
...
} else if (!has_moneyline_report) {
  message("moneyline_report() is unavailable; HTML export skipped.")
} else if (!has_moneyline_inputs) {
  message("Moneyline report skipped: comparison inputs were not generated earlier in the run.")
}
